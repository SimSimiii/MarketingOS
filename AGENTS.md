...
is clean. **Do not run `ruff format`**: the
...
CodeBuild specs run lint and tests. The frontend also has `npm run typecheck` and `npm test`.
Run the checks yourself before calling a change done. `.Codex/launch.json` defines `backend` and `frontend` for the preview tools —
...
- **The dev database is not Alembic-managed.** `init_db()` runs `SQLModel.metadata.create_all`
  ...
  Never stamp an old database at the new head: that skips the new columns. Back up first,
  identify the revision its existing schema matches, stamp that revision, then upgrade.
  See `docs/review-implementation.md`; `scripts/check_migrations.py` only touches a temporary DB.
