...
[docs/architecture-fonctionnelle.md](docs/architecture-fonctionnelle.md) is the shorter
functional counterpart, in French: what each phase does today and which module does it.
...
Lint `app tests scripts` — the same three the deploy buildspec lints, so a clean run here
is the one that matters. Not `.`: `alembic/versions/` is autogenerated and has never been
import-sorted, so linting the root reports ten `I001`s that are not yours. The baseline is
now genuinely clean, and it has to stay that way — ruff exits non-zero on a single finding
and `buildspec.yml` runs it in `pre_build`, so any finding left here fails the deploy.
**Do not run `ruff format`**: the repo is not format-clean (116 of 293 files would be
rewritten), so it buries a real change in an unrelated diff.
