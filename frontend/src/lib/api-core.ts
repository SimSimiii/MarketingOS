 ...
 *
 * The two callers answer this differently and cannot share an answer: a
 * browser reads a cookie the browser has, a server component reads the cookie
 * that arrived on the request it is rendering. Making it a parameter is what
 * keeps one definition of the sixty-odd calls below instead of two that drift.
 */
...
/** Thrown on a 401 so callers can tell "signed out" from "that failed".
 *
 * The distinction matters at exactly one place - the sign-in redirect - and
 * nowhere else, which is why it is a subclass rather than a second error
...
export function createApi(auth: AuthSource) {
  ...
  return {
    ...
    executionStreamUrl: (executionId: string, afterEventId?: number) => {
      // The token rides in the query string here, and only here (plus the CSV
      // export below). `EventSource` cannot set an Authorization header and
      // the API is a different origin, so the cookie the app holds is never
      // sent with it. The cost is a token that can appear in an access log,
      // which is why access tokens are short-lived; the alternative was
      // hand-rolling SSE over fetch and losing the browser's own reconnection
      // with Last-Event-ID, which is what makes a dropped connection resume
      // in the right place.
      const query = new URLSearchParams();
    ...
    /** What the sign-in page needs before anybody has a token. */
    getAuthConfig: () => request<AuthConfig>("/auth/config"),
    getCurrentUser: () => request<Account>("/auth/me"),
    listAuthSessions: () => request<AuthSession[]>("/auth/sessions"),
    changePassword: (data: ChangePasswordRequest) =>
      request<void>("/auth/password", { method: "POST", body: JSON.stringify(data) }),
    signOutEverywhere: () => request<void>("/auth/logout-all", { method: "POST" }),
    ...
    /** Describe a buyer yourself rather than searching for one. Spends nothing,
     * needs no compiled knowledge, and comes back with the same deterministic
     * admission receipt a mapped audience carries - which is the honest answer
     * to whether it can be researched yet. */
    addAudience: (brandId: string, audience: Partial<UserAudienceInput>) =>
      request<MappedSegment>(`/market/${brandId}/audience/segments`, {
        method: "POST",
        body: JSON.stringify(audience),
      }),
    /** Rewrite one audience you added. The name it is known by does not change:
     * research versions, prospects and campaigns all point at it by name. */
    updateAudience: (brandId: string, audience: Partial<UserAudienceInput>) =>
      request<MappedSegment>(`/market/${brandId}/audience/segments`, {
        method: "PATCH",
        body: JSON.stringify(audience),
      }),
    /** Take one audience you added off the map. Its research is left behind. */
    deleteAudience: (brandId: string, name: string) =>
      request<void>(
        `/market/${brandId}/audience/segments?name=${encodeURIComponent(name)}`,
        { method: "DELETE" },
      ),
